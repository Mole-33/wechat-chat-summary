...
async function saveProvider(silent=false){let baseUrl=$("providerBaseUrl").value.trim(),apiKey=$("providerApiKey").value.trim();if(/^(sk-|sk_)/i.test(baseUrl)&&!baseUrl.includes("://")){if(!apiKey)apiKey=baseUrl;const d=providerDefaults[$("providerKind").value];baseUrl=d?d[1]:"";$("providerBaseUrl").value=baseUrl;$("providerApiKey").value=apiKey;toast("检测到 API Key 被填在地址栏，已自动移到正确位置","error")}if(baseUrl&&!/^https?:\/\//i.test(baseUrl))throw new Error("API 地址必须以 http:// 或 https:// 开头，不能填写 API Key");const payload={id:$("providerProfile").value||undefined,kind:$("providerKind").value,name:$("providerName").value,base_url:baseUrl,api_key:apiKey||undefined,model:$("providerModel").value||$("providerModelList").value};const data=await api("/api/providers/save",{method:"POST",body:JSON.stringify(payload)});await loadSettings();$("providerProfile").value=data.provider.id;loadProviderForm(data.provider.id);if(!silent)toast("平台配置已保存","success");return data.provider.id}
