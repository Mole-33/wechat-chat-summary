...
const state = { settings: {}, groups: [], selected: new Set(), providers: [], jobId: "", lastJobId: "", jobPollFailures: 0, status: {}, startupNotice: "", monitorNotice: "" };
...
async function refreshStatus() {
  try {
    ...
    const monitorError = state.status.monitor_error || "";
    $("monitorBadge").className = `badge ${monitorError ? "badge-red" : (state.status.is_monitoring ? "badge-green" : "badge-gray")}`; $("monitorBadge").textContent = monitorError ? "读取异常" : (state.status.is_monitoring ? "读取中" : "已停止");
    ...
    if (monitorError && state.monitorNotice !== monitorError) { state.monitorNotice = monitorError; toast(monitorError, "error"); }
    if (!monitorError) state.monitorNotice = "";
    const resumableJob = state.status.active_summary_job_id || state.status.latest_summary_job_id;
    if (resumableJob && !state.jobId && resumableJob !== state.lastJobId) resumeActiveJob(resumableJob);
  } catch (_) {}
}
