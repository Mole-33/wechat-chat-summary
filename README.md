...
- 群聊选择改为点击展开的多选列表，可在列表中搜索并勾选多个群聊。

## v0.1.8 MiniMax 与接口稳定性优化

- 新增 MiniMax 内置平台，国内官方 OpenAI 兼容地址固定为 `https://api.minimax.cn/v1`，仅需填写 API Key。
- 支持从 MiniMax `/models` 自动获取可用模型，同时保留手动填写模型名称。
- MiniMax 总结前先校验 API、Key 与网络连接，失败时不会读取所选微信聊天正文。
- MiniMax-M3 总结默认关闭思考输出，减少等待时间并提高固定 JSON 结构的稳定性。
- 增强兼容接口错误解析，并忽略推理模型返回的 `<think>` 内容，避免干扰总结结构。

## v0.1.9 稳定性与任务恢复优化

- 防止重复点击、重复页面或刷新操作创建多个并发总结任务，避免重复计费与数据库争用。
- 页面刷新后会自动恢复当前总结任务及进度，不再丢失正在运行的任务状态。
- AI 平台遇到 `429/502/503/504` 临时错误时自动短暂重试；模型列表请求使用 30 秒超时并提供明确提示。
- 前端进度查询短暂中断时自动重试三次，不会立即把后台任务误判为失败。
- 定时总结改为按群依次执行，避免多个群同时调用 AI；内存中仅保留最近 20 个任务结果。
- 实时读取连续失败时，看板会显示“读取异常”并记录具体日志，不再静默显示为正常读取。
- 实时消息会在推进读取水位前排空全部分页，避免突发超过 500 条时漏读；运行中增删群聊不再回灌历史消息。
- 每次启动使用独立轮询停止信号，避免旧读取线程复活造成重复消息；多群消息按真实时间统一排序并保留最近 500 条。
- 修复无控制台便携版在数据库自动重建时因诊断输出不可写而中断实时读取的问题。
...
- 支持 OpenAI、DeepSeek、通义千问、智谱、豆包、硅基流动、MiniMax 及自定义兼容接口。
