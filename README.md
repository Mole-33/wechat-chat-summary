...
- 修复群聊多选下拉框被下方统计卡片遮挡的问题，展开后始终显示在看板内容上层。
- 下拉框会按窗口剩余空间自动向上或向下展开，并动态收缩列表高度，短窗口中也能完整操作。
- 支持点击外部关闭、Esc 关闭并把焦点返回触发按钮，同时补充控件关联与搜索框无障碍标签。
- 修复手机宽度下排行榜撑宽页面产生横向滚动条的问题，窄屏筛选控件会自动换行。
- 群成员名称固定按“群内昵称、微信昵称、成员 ID”显示，不再使用本机通讯录备注名。
