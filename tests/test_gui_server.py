...
class TestGUIServer(unittest.TestCase):
    ...
    def test_static_dashboard(self):
        ...
        self.assertIn('id="groupPanel"', html)
        self.assertIn('aria-controls="groupDropdown"', html)
        self.assertIn('role="dialog" aria-label="选择群聊"', html)
        self.assertIn('id="groupDropdown" class="group-dropdown hidden"', html)
        self.assertIn("toggleGroupDropdown", script)
        self.assertIn("positionGroupDropdown", script)
        self.assertIn('card.classList.toggle("group-menu-open", open)', script)
        ...
        style_response = urllib.request.urlopen(f"{self.base_url}/style.css")
        style = style_response.read().decode("utf-8")
        self.assertEqual(style_response.status, 200)
        self.assertIn("#groupPanel.group-menu-open{z-index:50}", style)
        self.assertIn(".group-select.drop-up .group-dropdown", style)
        self.assertIn(".dashboard-main,.dashboard-side", style)
